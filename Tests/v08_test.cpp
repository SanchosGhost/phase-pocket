...
static double bandGain(double frequency){constexpr double sr=48000.;auto e=std::make_unique<pocket::Engine>();e->reset(sr);e->configure(.65f,2000,20,20000,false,0,500,2000);double inPower=0,outPower=0;const int latency=e->latency();for(int n=0;n<48000;++n){float x=float(.5*std::sin(6.283185307179586*frequency*n/sr));auto v=e->process({x,x},{.8f,.8f});if(n>12000){float delayed=float(.5*std::sin(6.283185307179586*frequency*(n-latency)/sr));inPower+=double(delayed)*delayed;outPower+=double(v.out[0])*v.out[0];}}return std::sqrt(outPower/inPower);}
int main(){
    check(pocket::Engine::latencyForRate(48000)==48,"1 ms = 48 samples");check(pocket::Engine::latencyForRate(44100)==45,"44.1k latency rounds up");
    ...
    const double low=bandGain(100),inside=bandGain(1000),high=bandGain(8000);check(inside<.65,"selected processing band is reduced");check(low>.9,"low frequencies stay substantially dry");check(high>.9,"high frequencies stay substantially dry");
