...
void PocketLook::drawButtonText(juce::Graphics& g,juce::TextButton& b,bool,bool){
    auto r=b.getLocalBounds().toFloat();auto name=b.getButtonText();if(name=="Reset"){text(g,name,r,14,ink(),juce::Justification::centred);return;}
    ...
    if(name=="panel"){float dir=b.getToggleState()?-1.f:1.f;for(float y:{-4.f,3.f}){p.startNewSubPath(-5,y-2*dir);p.lineTo(0,y+2*dir);p.lineTo(5,y-2*dir);}p.applyTransform(juce::AffineTransform::scale(s).translated(c.x,c.y));stroke(g,p,ink(),1.7f*s);return;}
...
void ModernDial::paint(juce::Graphics& g){
    ...
    const bool showInfinity=infinity&&((infinityAtMin&&proportion<.0005f)||(!infinityAtMin&&proportion>.9995f));juce::String value;if(showInfinity)value=juce::String::fromUTF8(infinityAtMin?"-∞":"∞");else if(unit=="dB")value=juce::String(getValue(),2);else value=juce::String(getValue(),unit=="%"?1:0)+(unit=="%"?"%":" ms");
...
PhasePocketAudioProcessorEditor::PhasePocketAudioProcessorEditor(PhasePocketAudioProcessor& p):AudioProcessorEditor(&p),audioProcessor(p){
    ...
    gainWindow=preferences->getDoubleValue("phasePocket.ui.gainWindow",.5);scopeWindow=preferences->getDoubleValue("phasePocket.ui.scopeWindow",1.);
    setLookAndFeel(&look);setOpaque(true);setResizable(true,true);
    for(auto* c:std::initializer_list<juce::Component*>{&influence,&duration,&outputGain,&sidechainRange,&processingRange,&midSide,&settingsButton,&bypassButton,&panelButton,&resetFilter,&resetProcessing})addAndMakeVisible(c);
    ...
    bypassButton.setClickingTogglesState(true);bypassAttach=std::make_unique<juce::AudioProcessorValueTreeState::ButtonAttachment>(p.parameters,"bypass",bypassButton);settingsButton.onClick=[this]{showSettingsMenu();};panelButton.onClick=[this]{setPanelExpanded(!expanded);};
    ...
    for(auto* c:{static_cast<juce::Component*>(&sidechainRange),static_cast<juce::Component*>(&processingRange),static_cast<juce::Component*>(&midSide),static_cast<juce::Component*>(&resetFilter),static_cast<juce::Component*>(&resetProcessing)})c->setVisible(false);
...
void PhasePocketAudioProcessorEditor::paintOverChildren(juce::Graphics& g){
    ...
    const auto centre=blurArea.toFloat().translated(0,-float(blurArea.getHeight())*.09f);
