...
void glowText(juce::Graphics& g,const juce::String& s,juce::Rectangle<float> r,float size,juce::Colour c,int align,float amount){
    if(amount<=0.f)return;
    juce::GlyphArrangement glyphs;glyphs.addFittedText(uiFont(size),s,r.getX(),r.getY(),r.getWidth(),r.getHeight(),juce::Justification(align),1,1.f);
    juce::Path p;glyphs.createPath(p);
    stroke(g,p,c.withAlpha(amount*.30f),10.f);stroke(g,p,c.withAlpha(amount*.55f),5.f);stroke(g,p,c.withAlpha(amount),2.f);
}
