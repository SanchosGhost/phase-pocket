...
public:
 ...
 juce::AudioProcessorValueTreeState parameters;bool popTrace(PocketTrace&);
 std::atomic<bool> editorOpen{false};std::atomic<float> keyPeak{0},gainMeter{1};
private:
 pocket::Engine engine;
 std::atomic<float>* influence=nullptr,* smoothing=nullptr,* mode=nullptr,* scLow=nullptr,* scHigh=nullptr,* bypass=nullptr;
 void processAudio(juce::AudioBuffer<float>&,juce::MidiBuffer&,bool);double traceTime=0;
 juce::AbstractFifo fifo{2048};std::array<PocketTrace,2048> traces{};PocketTrace capture;int captured=0,decimation=40;
