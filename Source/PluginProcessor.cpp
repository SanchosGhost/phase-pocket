...
PhasePocketAudioProcessor::PhasePocketAudioProcessor():AudioProcessor(BusesProperties().withInput("Input",juce::AudioChannelSet::stereo(),true).withInput("Sidechain",juce::AudioChannelSet::stereo(),true).withOutput("Output",juce::AudioChannelSet::stereo(),true)),parameters(*this,nullptr,"PARAMETERS",layout()) {
    amount=parameters.getRawParameterValue("amount");duration=parameters.getRawParameterValue("duration");low=parameters.getRawParameterValue("scLow");high=parameters.getRawParameterValue("scHigh");bypass=parameters.getRawParameterValue("bypass");balance=parameters.getRawParameterValue("msBalance");
...
juce::AudioProcessorValueTreeState::ParameterLayout PhasePocketAudioProcessor::layout(){
    ...
    // Preserve parameter IDs/order for session loading; legacy controls have no DSP effect.
    ...
    p.push_back(std::make_unique<juce::AudioParameterFloat>("msBalance","Mid Side balance",juce::NormalisableRange<float>(-1,1,.001f),0.f));
...
bool PhasePocketAudioProcessor::isBusesLayoutSupported(const BusesLayout& l) const {
    ...
    return in==out&&(out==juce::AudioChannelSet::mono()||out==juce::AudioChannelSet::stereo())&&(key.isDisabled()||key==juce::AudioChannelSet::mono()||key==juce::AudioChannelSet::stereo());
}
...
void PhasePocketAudioProcessor::processAudio(juce::AudioBuffer<float>& b,juce::MidiBuffer&,bool hostBypass){
    ...
    displayBypass.store(hostBypass,std::memory_order_relaxed);engine.configure(amount->load()*.01f,duration->load(),low->load(),high->load(),hostBypass||bypass->load()>.5f,balance->load());
    bool show=editorOpen.load(std::memory_order_relaxed);float peak=0,minimum=1;double step=1./juce::jmax(1.,getSampleRate());
    for(int n=0;n<b.getNumSamples();++n){std::array<float,2> in{},sc{};for(int c=0;c<2;++c){in[size_t(c)]=main.getSample(juce::jmin(c,main.getNumChannels()-1),n);if(key.getNumChannels())sc[size_t(c)]=key.getSample(juce::jmin(c,key.getNumChannels()-1),n);}
        auto v=engine.process(in,sc);for(int c=0;c<main.getNumChannels();++c)main.setSample(c,n,v.out[size_t(c)]);
        minimum=juce::jmin(minimum,v.gain);peak=juce::jmax(peak,std::abs(sc[0]),std::abs(sc[1]));traceTime+=step;if(!show){captured=0;continue;}
        if(!captured)capture={v.dry[0],v.dry[0],v.key[0],v.key[0],v.out[0],v.out[0],v.gain,traceTime};
        else{capture.inLo=juce::jmin(capture.inLo,v.dry[0]);capture.inHi=juce::jmax(capture.inHi,v.dry[0]);capture.keyLo=juce::jmin(capture.keyLo,v.key[0]);capture.keyHi=juce::jmax(capture.keyHi,v.key[0]);capture.outLo=juce::jmin(capture.outLo,v.out[0]);capture.outHi=juce::jmax(capture.outHi,v.out[0]);capture.gain=juce::jmin(capture.gain,v.gain);capture.time=traceTime;}
        if(++captured>=decimation){int a,s,c,d;fifo.prepareToWrite(1,a,s,c,d);if(s){traces[size_t(a)]=capture;fifo.finishedWrite(1);}captured=0;}
