...
// Shared by the plug-in and DSP regression tests. No allocation in process().
namespace pocket {
...
class Engine {
    ...
    void frame() noexcept {
        ...
        for(int k=0;k<=N/2;++k) {
            ...
            auto& state=gains[k];
            state= g<state ? g : g+releaseFrame*(state-g);
            if(hz<20 || hz>20000) state=1;
            for(int c=0;c<2;++c){
                const double p=std::norm(B[c][k]); before+=p;after+=p*state*state;
                B[c][k]*=state; if(k>0 && k<N/2) B[c][N-k]=std::conj(B[c][k]);
            }
        }
        const float frameGain=before>1e-20 ? (float)std::sqrt(after/before):1.f;
        for(int c=0;c<2;++c) fft.run(B[c],true);
        // Negative-time zero-padded frames provide unity reconstruction from sample zero.
        for(int i=0;i<N;++i){
            const int dst=(outPos+i)%R;
            const float w=window[i]*0.5f;
...
public:
    ...
    void reset(double sr,float influence,int selectedMode) noexcept {
        rate=std::max(1.0,sr);pos=outPos=hop=priming=0;envelope=0;
        ...
        amount=targetAmount=std::clamp(influence,0.f,1.f);mode=targetMode=selectedMode?1.f:0.f;
        configure(influence,0,selectedMode);
    }
    void configure(float influence,float smoothingMs,int selectedMode) noexcept {
        targetAmount=std::clamp(influence,0.f,1.f);targetMode=selectedMode?1.f:0.f;
    ...
    Sample process(std::array<float,2> bass,std::array<float,2> kick) noexcept {
        Sample result;
        for(auto& v:bass) v=clean(v);
        for(auto& v:kick) v=clean(v);
        ...
        result.gain=1-amount*(1-((1-mode)*spectralGain+mode*ampGainDelay[pos]));
        ...
        for(int c=0;c<2;++c){
            ...
            result.out[c]=result.dry[c]+amount*(wet-result.dry[c]);
            ola[c][outPos]=0;
            input[c][pos]=bass[c];key[c][pos]=kick[c];ampDelay[c][pos]=bass[c]*(1-envelope);
        }
        ampGainDelay[pos]=1-envelope;
        pos=(pos+1)%N;outPos=(outPos+1)%R;
        if(++hop==H){hop=0;frame();}
        return result;
    }
};
}
