...
#include <algorithm>
namespace pocket {
// Two non-resonant TPT SVFs; 12dB/oct HP + LP, sidechain only.
...
public:
    void reset(double rate) noexcept {
        ...
        step=std::exp(-16/(sr*0.01));update();
    }
    ...
    void update() noexcept {
        ...
        const double a=targetLow>20.01?1:0,b=targetHigh<19999.?1:0;
        hpMix=a+step*(hpMix-a);lpMix=b+step*(lpMix-b);
        if(std::abs(hpMix-a)<1e-8)hpMix=a;if(std::abs(lpMix-b)<1e-8)lpMix=b;
        gh=std::tan(3.141592653589793*std::min(std::exp(lowLog),sr*0.45)/sr);
        gl=std::tan(3.141592653589793*std::min(std::exp(highLog),sr*0.45)/sr);
    }
    std::array<float,2> process(std::array<float,2> x) noexcept {
        if(count++==0)update();count%=16;
        for(int c=0;c<2;++c){double v=x[c],h=hp[c].tick(v,gh,true);v+=hpMix*(h-v);double l=lp[c].tick(v,gl,false);x[c]=(float)(v+lpMix*(l-v));}
        return x;
    }
};
inline float depthGain(float baseGain,float influence) noexcept {
    return std::clamp(1.f-influence*(1.f-baseGain),0.f,1.f);
}
}
