...
public:
    void reset(double rate) noexcept {
        sr=std::max(100.,rate);hpA={};hpB={};lpA={};lpB={};targetLow=20;targetHigh=20000;
    ...
    std::array<float,2> process(std::array<float,2> x) noexcept {
        ...
        for(int c=0;c<2;++c){
            const double dry=x[size_t(c)];
            const double h1=hpA[size_t(c)].tick(dry,gh,true);
            const double h2=hpB[size_t(c)].tick(h1,gh,true);
            double band=dry+hpMix*(h2-dry);
            const double l1=lpA[size_t(c)].tick(band,gl,false);
            const double l2=lpB[size_t(c)].tick(l1,gl,false);
            band+=lpMix*(l2-band);
            x[size_t(c)]=float(band);
        }
        return x;
    }
};
using SidechainFilter=SmoothStereoRangeFilter;
using ProcessingRangeFilter=SmoothStereoRangeFilter;
